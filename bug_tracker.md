...
- [x] **Feature**: 'Forgot Password' is a dead button. Show a SnackBar saying 'Please login via Google' (for future OAuth implementation).
- [ ] **Feature**: [TODO] Implement OAuth / Login via Google feature.
...
- [ ] **Bug** (Phase 2): State Disposal Issue. If I load the chat history, navigate to the Reminder screen, and go back to Chat, the history fails to load. Ensure that streams/sockets are properly re-initialized in initState after being closed in dispose.

## 6. Localization (i18n)
- [ ] **Feature**: Implement app-wide localization. Default language must be English, with an option in the Config screen to switch to Vietnamese.
- [ ] **Feature**: The selected UI language state must be passed to the AI backend so Shizuki automatically switches her spoken/written language to match the user's config.
